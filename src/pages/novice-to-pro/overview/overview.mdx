...
As a holistic framework, novice to pro is structured as 3 main components: **Experience playbook, components & implementation, and signal tracking.**
...
As users grow and mature with the product, so should the experiences they are continuously onboarded to. Upon the first-time use on a new experience, users are often inundated with a lot of information as they are trying to orient to a new ecosystem and find the first steps toward obtaining value. If everything is shown to the user upon this first-time engagement, the chances of retention and subsequent use are low. Timing and choreography when introducing new workflows are key because humans’ memory and cognition are limited.
