...
Use non-linear reading when space is limited to share a brief, at-a-glance,  summary of a concept that may require more explanation 
for some users. You can also use this component when the level of detail or the amount of context needed on a topic will vary across 
users and you don’t want to over-explain to more mature users. Non-linear reading is helpful to provide context for the first time a 
user engages with the product as well as for introducing new concepts later on as they get more familiar with base-level functionality. 

### When not to use

- Non-linear reading should not be used for defining terms (use the [definition tooltip](https://carbondesignsystem.com/components/tooltip/usage/) instead).
- It should not be used in place of documentation when the information is very complex or dense 
(instead, use non-linear reading to introduce a higher level detail with a link to 
read more in documentation). 
- It should not be used inside other components or areas where expanding the content would conflict with the component’s 
functionality (e.g. [coachmarks](/novice-to-pro/components/coachmark/usage)) or impede other 
elements on the page. 
- Do not use non-linear reading inside interstitial pages where content is meant to be a brief introduction of the value of the 
...
two lines.
2. **Expanded key word:** This is the selected state of a key word which includes a chevron to indicate it is expanded.
3. **Key word context:** When a key word is clicked, the additional detail is revealed. It is indented and in purple to distinguish 
it from the rest of the content and to maintain connection with the key word. Only the first level of key word context can contain 
additional key words.

### Placement

Non-linear reading is best used in components like the guide banner or inside inline tips, where space is limited. The height of the parent component will adjust as the non-linear key words are expanded.
...
Non-linear reading always begins fully collapsed, so that the non-linear key words are shown in their gray, minimized form.
