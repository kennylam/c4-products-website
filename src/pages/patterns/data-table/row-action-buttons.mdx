...
Action buttons and overflow menus are not interchangeable in terminology and are referred to as 2 different components in this document. <Link to="https://www.carbondesignsystem.com/components/button/usage">Action buttons</Link> (rather icon action buttons) are buttons that use icons to instantiate an action or lead to a flow via a dialog. <Link to="https://www.carbondesignsystem.com/components/overflow-menu/usage">Overflow menus</Link> always use the `...` icon to indicate multiple items are available.
