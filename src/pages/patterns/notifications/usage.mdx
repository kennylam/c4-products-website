...
Notifications provides a way to communicate product information to users. They range from granular, inline responses to user actions to system-wide messages
...
1. **Bell indicator**: Indicator serves as an indication that there are unread or new messages in the queue.

| **Item**                          | **Description (spec)**                                                                                                                                                                                       |
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| No messages in header (bell icon) | Will not show indicator, or red dot. User can still activate panel when there are no messages.                                                                                                               |
| Messages in header (bell icon)    | Will show indicator, or red dot, and not the count of messages. This was determined to be sufficient and more legible than a number indicating the amount of notifications (which could be multiple types).  |
...
1. **Bell indicator**: Indicator serves as an indication that there are unread or new messages in the queue.
2. **Dismiss all**: Dismisses all visible notifications in the panel. They are still available to view in the history page
3. **Settings icon**: Allows users to access the settings page directly from the panel
4. **View all CTA**: Allows users to access the notifications history page, and also shows a count of how many total notifications exist for a users account.
...
2. **ilters**: A user can access and build a set of their own filters for targeting more specific finding.
3. **Date designation**: All notifications grouped by day/date
4. **Find**: Ability to find any notifications you have.
5. **Delete group**: Deletes entire group or single notification within group. When selecting a notification or batch selection, the number designation appears along with the button becoming active.
6. **Notifications**: Selectable rows, sortable columns, and could be varying heights depending on the amount of information

**_Infinite scroll_**: Or as close to we can get as possible to abide by accessibility standards.
...
1. **Filter**: Based on type of notification
...
1. **Search**: Surfaces results based on most recent first, and then most relevant

### Notification open

![Notifications open](images/notification_open.png)

1. **Back button and toolbar**: Allows user to go back to history list and is the space for a potential tool bar if needed.
2. **Type/Title/Origin:** Shows what type of notification, what the title is, and where it's from in the lockup
3. **Pagination**: Allows user to paginate between notifications in this view
...
7. **Toggle switch**: Allows the user to choose how they want to receive notifications based on the offerings they have permissions to
...
- Use the view all page to show a users historical list of all of the users notifications or messages
...
- Do not use user account settings page pattern for admin settings page use cases
- Do not use admin account settings page pattern for user account settings page use cases
...
3. **Decision point CTA**: A call to action in order defer an immediate need for a certain amount of time with the understanding that the critical action needs to be taken eventually and the user will not be able to move further after a certain amount of deferments.
