...
- Once the user clicks the content/editable item, the Restore (Cancel) and Check (Save) icon buttons appear. 
- Once a change is detected, the Check (Save) icon button becomes enabled.
- If an item is not editable near other items that are, show the read-only (edit off) icon on hover. A tooltip should briefly state why it's locked. 
- To cancel the edits, the user can click the restore icon or press the Escape key to revert the field back to a static view and the latest value.
...
![Inline edit behavior](images/edit-inline-2.gif)

![Inline edit behavior](images/edit-inline-2.png) 
