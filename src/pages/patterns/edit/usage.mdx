...
- For example, for a read-only summary in a panel, clicking edit
  opens the edit modal (only for edits with 1-2 form fields).
...
1. **Title block**: The title block contains navigational elements such as closing, go back, and pop out, along with header text (always begins with "Edit [object name]") and a description that describes the side panel’s purpose.
...
4. **Primary actions**: For an edit side panel, the primary and secondary buttons are "Save" and "Cancel".
...
also being mindful of a size that comfortably and appropriately fits the content that
...
- The original create flow was in a narrow tear sheet and you don't want to obstruct the context by re-opening the narrow tear sheet for edits.
...
- The information being referenced is simple or if launching a simple create or edit flow.
- If your need a complex edit or create flow.
...
1. **Header**: The header can include a title, description, labels, and tabs. Always use "Edit" in the title.
...
1. **Editable content.** Everything on the page is editable and shown in the editable state.
2. **Save/cancel buttons.** Cancel changes and save changes buttons are always shown as disabled until
   a change is detected on the page. Note that "changes" are added to the button names in this case because
...
| The settings page is always editable | "Save changes" and "Cancel changes" buttons sit at the bottom of the browser window, and are disabled until change is detected. |
...
to make changes to a resource instance (ex. configuration details), account
...
aware of the consequences of the changes they made:

#### Low severity edits 

Changes that are only to a non-functional value and that
...
- Behaviors, assets, components, and workflows will need to change.
- The customer experience will change.
- The experience of other people in the organization will change.
- They will trigger the need to edit somewhere else.
...
Consider how the edits fit in the undo stack:
...
- If there is an "edit mode", consider allowing the user to undo specific element
  changes in addition to the overall undo after the user saves changes that would revert all of the changes made.
...
notification is not recommended to avoid excessive notification noise, as long as
...
| Auto save by interval         | System saves changes periodically without user action. Visual cues inform the user that changes are saved. | Use to prevent loss in case of error. Use when users are likely to remain in an "edit" flow for prolonged periods. Use when the application of changes does not disrupt the system. |
...
[warnings](#warnings) to inform the user of the consequences of their edits.
...
behavior than when the resource is created.

**Do: Use a variation that is best for the number of editable (or likely to be
edited) values.**  
Some variations (modal, inline editing) are best used when a few values are
...
Alert users when saving changes fail. If possible, retain their work and allow
...
values or the system as a whole.**  
Use steps, validation and progressive disclosure to help users avoid errors. When
