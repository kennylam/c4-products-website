...
Card nodes are a long-form visual representation best used when typography should be emphasized over icons. 
<Row>
...
The spatial proportions of the card node are based on that of the standard [Carbon notification](https://www.carbondesignsystem.com/components/notification/style/#structure). 
...
Card nodes can have four format variants: a bar that can be placed on the left side, top, or bottom of the node—depending on the direction of the represented flow, and an outline version that can be used for multiple card shapes, like rounded corners or pills. 
...
The default shape for card nodes is rectangular. However, outlined nodes can use a rounded rectangle or a pill shape. Only the rectangular-shaped card can be combined with a side or underbar.
...
In card nodes, status can be expressed in two different ways: one option is coloring the sidebar using the support tokens, and the other is using a status icon or shape in the upper right corner. When using the bar color to express status, make sure you have a secondary method of conveying the same meaning, using either text or the icon, to comply with color blindness accessibility standards. 
