...
- Use to add users and groups to a list of collaborators, recipients, etc.
...
- Don’t use this pattern for creating or editing a resource but you can use it in the context of these. Use the [Create flows](https://www.carbondesignsystem.com/community/patterns/create-flows/) pattern to create a resource.
- Do not mix the add pattern with editing an existing list of items. This is typically realized by Multi-select & batch action as covered in the Carbon [data table](https://www.carbondesignsystem.com/components/data-table/usage/#multi-select-and-batch-action) component. However, in simple cases where the items are represented in a compact list e.g. of tags you can use the Select flow - multiple items to edit the list. In many situations, the two actions (edit and add) differ as the list of possible items to add is significantly more than the number of items in the list to edit. Take the example of a list of collaborators in a project compared to the potential users in an organizational directory to add collaborators.
- Don’t use it when the items are from a small and easy-to-grasp list that does not require navigation and inspection. Use an Inline selection e.g. with a Dropdown or a Modal instead.
...
1. **Header:** Includes a title, optional label and description. The title should be brief and clearly describe the dialog’s task or purpose. Use the optional label above the title to set the context for the information in the dialog.
...
1. **Header:** Includes a title, an optional label, and an optional description. The title should be brief and clearly describe the dialog’s task or purpose. Use the optional label above the title set to the context for the information in the dialog.
2. **Search:** to find items in the list or hierarchy (optional: filter)
3. **Breadcrumb** (Add from hierarchy only): shows the context in the hierarchy and the user can navigate the hierarchy.
4. **List header:** shows context and count for the in the list, current location in the hierarchy, or search results.
...
6. **Drill down:** Indicates that an item can be navigated as a structural element in a hierarchy. Alternatively, a preview of the item’s content helps the user to make their decision.
...
9. **Modifier** (Add with modifier only): Include an optional property the user can change defining the detail of how the item is added e.g. the role of a collaborator. The modifier appears on hovering row only.
...
11. **List of selected items:** lists items with additional information and modifiers. Items can be directly removed from the list. They can be opened to preview content if this helps the user to make their decision.
...
For multi-select, make sure you always use a large tearsheet and include a side panel so the user can see which items were selected and can see the selection at a glance before adding to a list.
...
| Add items hierarchical           | Wide tearsheet      | Use when the user needs to find multiple items in a hierarchy by scrolling, navigating and searching. In some use cases, the wide tearsheet can be used also for single-select flows – but only when adding an item involves further steps or significant resources or if the user needs further context of the items to make an informed decision. See section additional content or navigating a hierarchy.             |
| Add items with modifier           | Wide tearsheet      | Use when adding items from a large list or hierarchy – optionally with preview – when the user can specify a detail how to add each item. |
...
A **select flow** is usually started when the user clicks on a button to set, select, or change an item referenced in an asset or settings page. An **add flow** usually starts when the user clicks on an add button to add one or multiple items to a list in an asset or settings page. 
...
<Caption>
  {' '}
  After the user selects an item, the action button changes from “select” to “edit”.  After clicking the edit   button, the user can select a different item and replace the current one.{' '}
</Caption>
...
<Caption>
  {' '}
  If multiple items should be added, it is recommended to use the data table empty state as an entry point. The primary action button on the table triggers the tearsheet/add-flow. Single selections should not be displayed in a table view.
{' '}
</Caption>
...
In a **select flow**, the user picks an item and confirms the selection using the Select primary button. The primary CTA button label in select-flows is “Select” (or “Select & replace”). In an **add flow**, the user selects items that collected in a list of selected items. At any time and typically before completing their task the user can review and adjust the selected items. Eventually, users complete the task by confirming to add the selected items using the CTA button. The primary CTA button label in add-flows is “Add”.
...
When the user clicks the “Select” or “Add” button, consider using an indication of loading or submitting if the operation may take longer. See Carbon’s Loading Pattern. Also, see specific guidance for latency (loading) in the Tearsheet component.
...
##### Tag to indicate the amount of selected items
...
<Caption>
  {' '}
  The user can also remove already selected items by clicking the remove icon. The remove action icon shows only upon hovering the row to avoid unnecessary visual clutter and place the destructive action on a secondary level. Alternatively, the user can also deselect the checkbox on the left side of the list.
...
##### Expand additional information about a selected item
...
<Caption>
  {' '} Users can reorder the selected items via drag & drop if the order of the items matters to the user. This might not be needed for all use cases and is optional. Styles and interactions follow the reorder pattern.
{' '}
</Caption>
...
<Caption>
  {' '}
  In an add flow, already added items are shown as selected but disabled. This anticipates that the existing list might be large and the user not necessarily know all already added items by heart.
{' '}
</Caption>
...
#### No access to data
If users don’t have access to data (e.g. there are no items available or the user don’t have permission to access items, we recommend disabling the action button already on the screen before to avoid an additional step to enter the tearsheet.
...
To hint scrolling the pattern hints half a row behind the fix footer bar. This behavior helps the user to get a better idea about the content before discovering items using search.

<br />

### Navigating a hierarchy

In a hierarchy items in a list may group further items. For instance, you may have a geographical hierarchy with continents, countries, states and cities. When focusing on a continent on the top level, you see the second level with its countries. Focusing on a country opens the next level of states. And finally, you will access the fourth level of cities. An arrow next to an item indicates the further level of items and users can navigate into items by clicking either the name (link) or chevron right (icon) in a column.

<br />

#### Usage in narrow tearsheet

Use a narrow tearsheet when the user needs to find a single item in a hierarchy. Narrow tearsheets are reserved for single-select-flows. For multi-select, make sure you always use a large tearsheet and include a side panel so the user can see which items were selected and can see the selection at a glance before adding to a list. 
...
Use when the user needs to find multiple items in a hierarchy. In some use cases, the wide tearsheet can be used also for single-select flows – when selecting an item involves further steps or significant resources or if the user needs further context of the items to make an informed decision. 

Breadcrumbs are used to navigate between columns to reflect the current location relative to the column. That way users are able to also jump from a very deep level back to the first level without losing context. Navigating between the columns does not influence any selection a user makes. The selection a user makes stays as-is. Once found the user selects the items. This enables the Add button. 

![Image](images/image_Add_individual_items.png)

<br />

*This pattern for navigating hierarchies can be also applied in other areas of the UI (e.g. full pages).*

<br />

#### Variations

1. Add a single item from the hierarchy
2. Add a parent item with all associated children
3. Add only specific children of a parent item
4. Non-hierarchical with a peek inside item (Example user groups)

<br />

#### 1. Add a single item from the hierarchy: 
In some use cases, the wide tearsheet can be used also for single-select flows – but only when adding an item involves further steps or significant resources or if the user needs further context of the items to make an informed decision. Additional data about the select item can be displayed in the side panel to provide more context about the selection.
...
#### 2. Add a parent item with all associated children:
Use this variation when all items of a parent category should be automatically selected. Make sure to include only the parent items in the selected panel, don’t list all the individual items in the panel. For this use case, consider adding a **Select all** option to the column toolbar.
...
- In a select flow, the currently selected item is shown as selected but are disabled. 
- In an add flow, already added items are shown as selected but is disabled. This anticipates that the existing list might be large and the user not necessarily know all already added items by heart.

<br />

#### 3. Add only specific children of a parent item:
Use this variation when only specific and not all items of a parent category should be selected. All individually selected items are listed in the sidepanel. 
...
#### Searching a list
In large lists, search is the primary means of discovery. When the dialog opens the focus is set on the search field. Once the user provides a search text the list heading changes to indicate that the list represents the search results. 

![Image](images/image_search_results.png)

Also in a hierarchical structure, the search is “global” and returns items across the whole hierarchy (all columns). The user can navigate into structured elements within the search results. The breadcrumbs reflect the current location relative to the search result as a starting point.

If users deal with large amount of data are required to search within a specific level of hierarchy (column) see the section below **Search, filter & sort within one hierarchy**.

<br />

#### Filtering
If users deal with a large amount of data in a list, it helps to add filtering to find relevant items quicker. Follow guidelines for filtering. 
...
If there are more than two levels the user can drilldown in a hierarchy. Especially if there are large amounts of data, users require an option to filter and sort specific columns. This helps them to easier find an item. Use sorting & filtering only when the hierarchy has more than two levels. For two levels the global search is sufficient.
...
<Caption>
  {' '}
  When the user hovers over a row modifier is shown for that row
  {' '}
</Caption>


In complex situations consider a multi-step tearsheet with a progress indicator separating the steps of adding items and setting and changing their aspects. However,  if the user can set and change only one aspect using an inline input field, use a modifier for this aspect.




#### Set and change a modifier
...
<Caption>
  {' '}
  Modifier without a default value
  {' '}
</Caption>
...
Click on a row or selecting the eye icon triggers additional information about the selected item in a sidepanel or stacked tearsheet
<Row>
  <Column colMd={4} colLg={4}>
...
<Caption>
  {' '}
  Click on a row triggers more information about the list item in the RHS panel. Due to discoverability the user also gets an icon on hovering the row.
{' '}
</Caption>
...
| Stacked tearsheet |
| ----------------- |
| Use for more complex data that need to be shown in the context of the flow to the user (e.g. a preview of a PDF, data set or a notebook). |
...
| Sidepanel |
| --------- |
| Use for simple information that adds additional value to the user and helps in the decision-making for the selection (e.g. metadata). | 

![Image](images/image_preview_content.png)

<Caption>
  {' '}
  Additional information in the side panel (e.g. metadata)
{' '}
</Caption>
...
- Call out the name of the asset in the header to give the user enough context as what item on the page the metadata relates to. 
...
- Avoid adding any interactive links in that panel, as this would pull users out of their current work context and bring them out of the tearsheet to a separate page which interrupts their current task of adding items. 

To indicate to the user which row is selected and to which item the information in the panel relates, we add an indicator line (blue, 2 px on the far left of the row). Follow the guidelines of the clickable row items data table pattern. When the panel is triggered the panel covers the selected panel with the primary action buttons. This prevents users from being misled and might assume they only add one single item. Action buttons for adding are only displayed when the whole selection is visible to the user. 
...
By default, the focus should be on the global search input field as searching for items is the primary action a user does when performing an add/select action from a large list or hierarchy.

<br />

### Table row interactions
Follow the data grid pattern from WAI-ARIA which has several keyboard commands or behaviors that make tables easier to navigate:
