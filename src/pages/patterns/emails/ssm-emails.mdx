...
7. **Custom text**. This component contains optional custom text, which should be brief. For example, "Refer to the product documentation for usage details." - where "product documentation" is a clickable link. 
8. **Transactional email footer**. This component provides access to required legal information and notice for the transactional class of business emails, including the IBM Privacy statement.
...
The next email in the SSM flow provides notice to the user that IBM has deleted their trial. If the user has already upgraded their subscription (by purchasing a subscription within the grace period), then this original trial may or may not be deleted (the user may have opted to continue with their trial environment after purchasing, for example). A common grace period following trial expiration is 30 days, enabling both your product team and the user to continue using the product with minimal intervention. The CTA buttons enable the user to purchase (primary action) a new subscription or to contact IBM sales (secondary action) for additional purchase options.
...
The next email in the SSM flow thanks the user who has upgraded their Try offer, typically by purchasing a subscription. The CTA buttons enable the user to get started (primary action) with their upgraded product subscription or to manage their subscription (secondary action) at My IBM. 
...
Note: An HTML snippet of the (transactional email footer](transactional-email-footer#code-snippet) is provided.
...
<iframe class="airtable-embed" src="https://airtable.com/embed/appvxykrpLizynCkz/pagLdSFicEPM1jOp1/form" frameborder="0" onmousewheel="" width="100%" height="600" style="background: transparent; border: 1px solid #ccc;"></iframe>
