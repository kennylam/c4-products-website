...
<AnchorLink> Your Try instance has been deleted</AnchorLink>
...
MCSP transactional Try emails components are shown in the following Welcome email example: 
...
3. **"Seller account** This component contains the customer's account number as held by the product seller. "Seller" in this case is the software retailer which provided the user's subscription - either IBM or a third-party software seller such as AWS. 
...
5. **CTA buttons**. This component contains call-to-action (CTA) buttons which prompt user action. CTAs are the primary (blue) action the user should do, and optionally, a secondary (white) action. If there is no user action required, as with the "Thank you for subscribing" email, no CTAs are used.
...
Your product team should consider the following additional guidelines when implementing the MCSP trial transactional email flow: 

- The email "Subject" line is important email-specific text which appears in the recipient's email in-box and should be implemented as described. 
- When a user purchases the IBM MCSP product, MCSP Try emails should no longer be sent. Transactional emails for their purchase are then sent instead of Try emails. If the user upgrades to another Try offer for the product, the MCSP Try offer email flow should restart for the new subscription ID.
