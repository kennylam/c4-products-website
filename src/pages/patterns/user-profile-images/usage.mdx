...
Our products need user profiles that display a representation of users. This can be in the form of:
...
For the user’s initials for the avatar, we use both the first and last name of the user. This provides a recognizable avatar in isolation, as well as in context. 
...
Ideally, we would use user avatars if users don’t upload a profile image. For products that have technical constraints in displaying avatars or initials, the user profile icon for single users can be used. We are also pairing the icon with a color that visually organizes comment threads or lists better.
...
We use 6 different sizes for user profile images. 64px, 48px, 32px, 24px, 20px, and 16px. Text and icons are always centered in the circle and use the following sizes: 
