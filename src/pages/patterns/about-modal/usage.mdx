...
5. **Links (_optional_):** User links if there is additional information to call out within the card. The About modal should be used to display the product information and not where users go to find help
6. **General text (_optional_):** Subhead text providing any relevant product disclaimers including legal information.
7. **Trademark and copyright information:** Displays the first year of product release to current year.
...
Inside of an about modal, body copy, including titles, has a 20% [margin-right](https://www.carbondesignsystem.com/components/modal/style#margin-right)
...
All card titles, subtitles, labels and descriptions are set in sentence case. Set titles, subtitles and body text appropriately based on content.
...
Since cards present different types of content, some spacing between the elements can vary according to the context. The following specs present some of the constant spacing, such as internal padding, space between images, title, description, and links.

![About modal spacing tokens](images/14.png)

#### Light option

By default, we recommend each product use the dark theme for their about modals as they are triggered from the About section within the profile menu in the main navigation. However, you may opt to choose the light version if this is a product requirement.
...
The about modal triggers a state (or mode) that focuses the user’s attention exclusively on one task or relevant information. When a modal is active, the content of the underneath page is obscured and inaccessible until the user completes the task or dismisses the modal.
...
Microsoft Docs, [About Dialog Boxes](https://docs.microsoft.com/en-us/windows/win32/dlgbox/about-dialog-boxes) (2019)
