...
locally to select a file. Additionally, provides the ability to add a file from
...
If additional actions (e.g. adding metadata) are required to be performed on the
file(s) they should be done separately from the file upload, prior to the import
...
applies to any single file upload or import interaction.
...
1. On entering an Import section or page, focus on the first `Tab` keystroke is
   ...
   file uploader.
2. On `Tab`, focus is applied to the URL field.
...
between the selected files remove actions, and move to the next section of the
page.
