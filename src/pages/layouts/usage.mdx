...
1. **Top spacer (optional):** Offsets the layout module from the object positioned directly above it. [Content layout modules](#content-layout-modules) often use a top spacer. If a top spacers is include, `$spacing-05` is typically the token used.
2. **Title section (optional):** Layout modules can include nested titles or sub-titles. Reference the `titleModule` for type styles and tokens.
3. **Component spacer (optional):** Offsets multiple objects using `spacing tokens` within a single layout module.
4. **Main content:** The core component or attribute of the layout module.
5. **Bottom spacer:** Creates padded space between the layout module and the object positioned directly below it. Bottom spacers typically use `$spacing-05` or `$spacing-07` depending on the module.
...
<Video src="images/module-variants.mp4">
  <track kind="captions" src="images/module-variants.mp4" />
</Video>
