...
<strong>branch</strong> (noun)<br />
  ...
  - It is contained within the repository, but does not affect the primary ('main') branch, allowing you to work freely without disrupting the main version.<br />
  - When you've made the changes you want to make, you can merge your branch back into the main branch to publish your changes.<br />
...
<strong>pull request (PR)</strong> (noun)<br />
  ...
  <AccordionItem title="Understanding the GitHub workflow">
    ...
    This page covers the process from creating a branch to committing changes to opening a pull request to changes being merged into the main branch.
  </AccordionItem>
</Accordion>
...
- If you are already _in_ the main branch, run the following commands:
  ```
  git fetch
  ```
  ```
...
- You may be required to enter a merge message. If so, press **Enter** on your keyboard until you reach the `#` line. 
  You can enter `:wq!` to exit this stage and all changes from main will now be in your repository.
...
you forgot to create a new branch and have made changes in the `main` branch), this handy command will 
help.
...
1. First, pull the latest changes from the main repository.
