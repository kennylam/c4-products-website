...
3. **App icons:** App icons can be used to further indicate the link. If app icons are in use, they must be displayed consistently throughout all links at a size of 32 x 32px. Reference the [App icon guidelines](https://www.ibm.com/design/language/iconography/app-icons/library).
...
<Caption>*Exception: Card displaying an ICA (big number values) can scale down to a minimum width of 2 columns.</Caption>
...
The ‘Overview’ using the masonry layout framework is ready for production. For any questions, please contact:
...
The ‘Optional views’ and the collage layout framework is work in progress. For any questions, please contact:
