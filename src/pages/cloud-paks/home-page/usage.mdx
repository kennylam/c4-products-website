...
4. **App icons:** App icons can be used to further indicate the link. If app icons are in use, they must be displayed consistently throughout all links at a size of 32 x 32px. Reference the [App icon guidelines](https://www.ibm.com/design/language/iconography/app-icons/library).
5. **Hero image:** The hero image differentiates the homepage from other pages. The illustration should tell a meaningful story or explain the functionality of the Pak. The hero image should not interfere with the links. Max width spans to 562px on a 1312px width screen. Reference the [isometric illustration guidelines](https://www.ibm.com/design/language/illustration/isometric-style/design).
6. **Accordion button:** The accordion button collapses and opens the welcome section. The collapse state displays the Welcome text, custom logo (if there is one), and accordion button. The last state is persistent even after the page refreshes. The state only changes when the user interacts with the accordion button.
