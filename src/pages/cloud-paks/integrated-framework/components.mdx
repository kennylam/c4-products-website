...
<Row className="resource-card-group">
  <Column colMd={4} colLg={4} noGutterSm>
    <ResourceCard
      ...
      actionIcon="download"
      aspectRatio="2:1"
      href="https://ibm.box.com/shared/static/pop95z30sbjvhi7v9oqegcxmrcu24hxl.sketch">
...
Pivot buttons provide the ease and means to contextually work from one application to another application, keeping the user in a fluid workflow. Pivot buttons can be used as a [single button](https://www.carbondesignsystem.com/components/button/usage/) action or presented as a [combo button](https://pages.github.ibm.com/ai-applications/design/components/buttons/usage#split-button) when multiple options are available.

### Anatomy

Pivot buttons leverage Carbon buttons. Pivot buttons augment Carbon buttons with the use of application icons to set destination expectations. This anatomy example uses a [combo button](https://pages.github.ibm.com/ai-applications/design/components/buttons/usage#split-button).

<Row>
 <Column colLg={8}>

![Pivot button anatomy](images/pivot-button-anatomy.png)

 </Column>
</Row>

1. **Available actions:** Supplementary apps that perform an action or take the user further in to the experience to obtain information about the subject.
2. **Upsell nudge (optional):** Apps that are not available to the user but can perform an action or provide a point of view about the subject. To avoid over exposure to upsell nudges in productive use, conditionally considered use for appropriate contexts (ex: Trials. First 30 days of productive use).
3. **Default action:** Primary app action for the subject.
4. **Text label:** Indicates what action will occur. The application name should not be included in the text label. Button labels should follow <Link to="https://www.carbondesignsystem.com/components/button/usage/#text-labels">Carbon text label guidance</Link>, using the {verb} + {noun} formula and leverage common <Link to="https://www.carbondesignsystem.com/guidelines/content/action-labels">action labels</Link>.
5. **App icon:** Indicates the app where the action will take place or where the user will be taken. The solid color icon is predominantly used within buttons unless a ghost button is used. Ghost buttons use the color icon. For better legibility, icons are size 20.
...
<DoDont caption="In addition to standard buttons and combo buttons, pivot buttons can be included within overflow menus.">
...
![ReturnTo banner anatomy](images/returnto-banner-anatomy.png)
...
A decorator is composed of a severity indicator, an object type, and a value. Everything but the value is optional. There is a spacer line used only when the indicator and/or the type is displayed to separate the value.

<Row>
 <Column colLg={8}>

![Data decorators anatomy](images/data-decorators-anatomy.png)

 </Column>
</Row>

1. **Indicator shape (optional):** The severity of the object. (ex: critical, high, medium, low)
...
1. **Indicator + value:** Use this variant when the type is a label or when it is used within a table column and the column header is the object type.
...
<Video src="images/data-decorators-panel-interactions.mov">
  <track kind="captions" src="images/data-decorators-panel-interactions.mov" />
</Video>
...
Actions can be taken directly from a data decorator using right click to initiate a context menu. The contents of the menu can include actions to be taken within the current page as well as pivot actions to continue analysis across other applications within the platform.

<Row>
 <Column colLg={8}>

<Video src="images/data-decorators-context-menu-interactions.mov">
  <track kind="captions" src="images/data-decorators-context-menu-interactions.mov" />
</Video>
...
![Data fragments anatomy](images/data-fragment-anatomy.png)
...
3. **Fragment content:** Information related to the subject, sourced from another application(s) or connected data source(s).
...
![Data decorators anatomy](images/contextual-tearsheet-anatomy.png)
...
<DoDont caption="This use case demostrates that an “artifact” can be captured from any app. The platform brokers the exchange using the tearsheet to house the experience from the sending and receiving apps to save the artifact to the receiving app.">

![This use case demostrates that an “artifact” can be captured from any app. The platform brokers the exchange using the tearsheet to house the experience from the sending and receiving apps to save the artifact to the receiving app.](images/tearsheet-howto-1.jpg)
...
![Data decorators anatomy](images/modal-anatomy.png)
