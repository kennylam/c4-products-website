---
title: File Uploader
tabs: ['Usage']
---
<InlineNotification kind="warning">

  This page provides usage guidelines to be used in conjunction with the [Carbon File uploader component](https://www.carbondesignsystem.com/components/file-uploader/usage).  
  
</InlineNotification>

### Uploading is transferring a resource from a local system to a remote system.

<AnchorLinks>
  <AnchorLink>Overview</AnchorLink>
  <AnchorLink>Location</AnchorLink>
  <AnchorLink>Upload methods</AnchorLink>
</AnchorLinks>

<Row>
  <Column colLg={8}>

![Example of a file upload in a full page](images/0.png)

  </Column>
</Row>

### Things to consider

- Is the action uploading or is it importing?
- What are the allowed upload methods?
- How many files are likely to be uploaded?

### Overview

The action of uploading should always be the result of a user action.
The action of uploading a file could be done by selecting an action button
to open a modal or inline in a full page form, slide-in or slide-over panel.

<Row>
 <Column colLg={8}>

![Example of an upload view](images/1.png)
<Caption>The user lands in an upload view</Caption>

 </Column>
</Row>
<Row>
 <Column colLg={8}>

![Uploaded files are listed below the controls and are validated.](images/2.png)
<Caption>Uploaded files are listed below the controls and are validated against requirements.</Caption>

 </Column>
</Row>
<Row>
 <Column colLg={8}>

![The user can continue when the required file(s) are uploaded.](images/3.png)
<Caption>The user can continue when the required file(s) are uploaded.</Caption>

 </Column>
</Row>

### Location

Upload can take place in a modal, full page view, or in a side panel. 
Do not use upload in a modal when multiple files are uploaded. 

<Row>
 <Column colLg={8}>

![Uploading in a modal with a maximum of one file.](images/4.png)
<Caption>Uploading in a modal with a maximum of one file.</Caption>

 </Column>
</Row>
<Row>
 <Column colLg={8}>

![Uploading in a side panel](images/5.png)
<Caption>There is no maximum number of files in a side panel upload.</Caption>

 </Column>
</Row>

<Row>
 <Column colLg={8}>

![Full page uploading](images/6.png)
<Caption>There is no maximum number of files in a full page upload.</Caption>

 </Column>
</Row>

### Upload methods

- Drag and Drop
- Choose a file
- Specify a URL

Wherever possible there should be a drag and drop zone, using the Carbon File
upload component, which also allows the user to click and browse local folders to select a file.
Note that file selection should be restricted to only allowed file types.

<Row>
 <Column colLg={8}>

![Where users can upload from non-local locations, provide a URL entry box.](images/7.png)
<Caption>Where users can upload from non-local locations, provide a URL entry box.</Caption>

 </Column>
</Row>
<Row>
 <Column colLg={8}>

![Other upload methods are disabled when the user interacts with one.](images/8.png)
<Caption>Other upload methods are disabled when the user interacts with one.</Caption>

 </Column>
</Row>
