...
The _Additional tag feature_ allows users to see all the labels of a cluster in areas where all tags may not be visible at one time. They provide the ability to see how many additional labels exist and what they are.
...
The **Addidiontal tag** feature is used when not all the tags fit in the allotted space. It allows users to see all the labels associated with a cluster. This additional tag feature looks like a regular tag but is indicated with a plus sign and a number (the number of additional labels not seen). 
