...
3. **Summary (optional):** A summary of the current state of the content
...
#### Toggle (optional)
- Use options tiles with a toggle when the entire content can be changed between enabled and disabled state.
- The tile can still be expanded when the toggle is turned off but all controls inside should be disabled.
- When the tile is in disabled state, the summary is hidden (except for [validation states](#validation)).
...
#### Mouse
By clicking anywhere on the header of an options tile, it expands when it is collapsed and collapses when it is expanded.
...
#### Keyboard
The header element is in the tab order and receives focus. Pressing either <kbd>Space</kbd> or <kbd>Enter</kbd> will expand or collapse the options tile.
...
#### Screen readers
- **VoiceOver:** Users can expand or collapse the tile by pressing <kbd>Control</kbd>+<kbd>Option</kbd>+<kbd>Space</kbd>, <kbd>Enter</kbd> or <kbd>Space</kbd> while the header has screen reader focus
...
#### Locked
When all controls inside an options tile are disabled due to a lack of permission, a lock icon is displayed next to the summary line. Additionally, when expanded, the reason for the locked state is explained to the user.
...
If multiple options tiles are related to each other, consider placing them directly adjacent / underneath each other to create a visual group and separate multiple groups by additional spacing and ideally a heading describing the group.

<Row>
  <Column colLg={12}>

![Example of grouping](images/ux-grouping.png)
