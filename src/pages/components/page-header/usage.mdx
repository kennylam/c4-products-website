...
6. **Actions (optional):** This zone is for placing critical page actions. While this zone is primarily for buttons, the specific type of component is left to the product team's discretion to meet user experience needs. Some examples of appropriate components within the action zone would be buttons, dropdowns, and overflow menus.

## Types


### Page header with background
This is the primary expression of the page header. The background provides structure and aids in establishing a hierarchy for page headers with more items. The page header with a background should be used if tabs, subtitles, or more global items are required.
...
#### With tabs & tags
If both tabs & tags need to be present, tabs should remain left-aligned with the page and tags repositioned to the right side of the screen.

![with tabs tags](images/with-tabs-tags.png)

#### With subtitle
Subtitles, if necessary, are placed directly below the page title. Page headers with subtitles use the background to create a hierarchy and to separate it from the type placed within the page.

![with subtitle](images/with-sub-title.png)

#### With summary details
Page content with a global page hierarchy can be placed above tabs within the page header. This is an example. The actual page content should be determined by the offering team.

![with summary details](images/with-summary.png)

#### With the breadcrumb + actions toolbar
...
#### With the actions toolbar only
Some pages may require maximum utilization of the browser window to create a full canvas experience. For experiences that meet this specification, the page header can default to the collapsed state. 
...
### Editable page title
On occasion, renaming the current page title is required. This optional interaction sources the Carbon for IBM Products inline edit pattern. Additional guidance can be found in the [Edit and update section](https://pages.github.ibm.com/cdai-design/pal/patterns/edit/).
...
If the page header includes a toolbar, the buttons scroll with the page next to the page title. As the buttons scroll out of view, they should fade into view in the toolbar next to the actions.
...
2. The second option pushes the breadcrumb out of view, replacing it with tabs on scroll. This should only be used for stage space-saving needs. The initial page title transition remains the same. After this initial transition, the tabs push the breadcrumbs out of view as the page is scrolled further. As soon as the page is scrolled towards the top, the breadcrumb row is again stacked above the tabs. 
...
- Where possible, move metadata out of the header and either onto the page or a “details” button
...
- [Toolbar](/patterns/toolbars/usage)
