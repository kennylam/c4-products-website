...
This pattern explains how to lay out content within an expanded area and why content within the expanded area cannot be aligned to the grid and should not be aligned with table columns.
...
To prevent a vertical scroll bar in the expanded area, the area does not have a `max-height`. However, you should consider the following:
...
If you are working in Sketch it is easy to create a design where items in the expanded area align to the grid or the table columns. However, as soon as a developer attempts to implement this design they will quickly encounter problems.

Instead of aligning content to table columns or the grid, you should lay out your content in a manner that establishes clear relationships between content items and utilizes Carbon's [layout scale](https://www.carbondesignsystem.com/guidelines/spacing/overview/#layout-scale).
...
You should only wrap the items in the expanded area if you can preserve important relationships between content items and ensure that the expanded area is still usable. If this is not possible, consider displaying the content differently.

In the _do_ example, all of the items in the expanded area wrap into one column. This prevents the _Provision Status_ section's placement from bisecting the _Details_ section in a way that makes it difficult to tell that the _Details_ section contains key-value pairs.
...
- [Data table](https://pages.github.ibm.com/cdai-design/pal/components/data-table/overview)
