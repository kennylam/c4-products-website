...
searching is meant to help the user find a specific result, whereas filtering allows users to trim results according to their attributes.


Applied filters can affect both the data that is visible in the table and data that might not be displayed in the columns 
...
1. **Filter button:** Icon button used to open the flyout and display all available filter options.
...
1. **Filter button:** Icon button used to open the flyout and display all available filter options.
2. **Filter header:** Defines the function of the filter flyout or panel.
3. **Accordion (optional):** Collapsible sections that allow up to 10 filter values to be displayed.
...
11. **Tearsheet (optional):** If the number of options for a particular filter is too high to list in the panel, use the tearsheet* to 
...
2. **Filter tags:** Each filter is applied to the table with the option to remove the filter. 
...
4. **List all button:** Expands the filter summary to show up to two lines of filter tags. This button only appears if applied filters exceed one line.
...
![Don't group multiple filters in one tag.](images/Filters_23-DoNot-02.png)
...
#### Instant updates
The instant update method returns results after each selection is made. This method is preferred because it allows users to 
...
has a minimum height of 21rem (336px), and the buttons will stop sticking to the bottom of the browser at this point to not overlap 
...
#### Filter panel
If filtering is a common action and users need to see the filters applied to the table as they are making their selection, the 
...
#### Filter summary
The filter summary is placed between the toolbar and the table body.
...
#### Filter flyout 
The filter flyout width can be set for different breakpoints, with filters reflowing into a single column as needed. On 
