...
If there is limited space or on smaller screen sizes, we recommend using only an overflow menu that lists all possible actions.

![Image](./images/image_box-image-88.png)

### Actions across multiple pages
By default batch actions always select the current view of items on the first page. When users deal with large amounts of data and want to perform batch actions, it can become time-consuming to select items only from the current page and perform the same action multiple times for all other pages. Therefore users should be able to select all items across multiple pages for related objects in a table.


When a table has a large number of items (+100 rows) we recommend using a Select all button next to the indicator, that shows how many items currently are selected. 
...
- When an action cannot be undone, it would be very expensive or time-consuming to re-create data. 
