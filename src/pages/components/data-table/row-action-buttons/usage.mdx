...
Action buttons and overflow menus are not interchangeable in terminology and are referred to as 2 different components in this document. [Action buttons](https://www.carbondesignsystem.com/components/button/usage) (rather icon action buttons) are buttons that use icons to instantiate an action or lead to a flow via a dialog. [Overflow menus](https://www.carbondesignsystem.com/components/overflow-menu/usage) always use the `...` icon to indicate multiple items are available.
...
- The overflow menu is the default to use if there are more than 2 actions a user takes on a row.
- When fitting to mobile screens, convert any number of action buttons in a row to persistent overflow menus.
...
- Show action buttons persistently only if the user is being encouraged to click them. Otherwise, action buttons should be visible on hover to discourage use and de-clutter content that would otherwise become redundant.
...
- When a table has rows that include actions which are dependent on other properties that can vary the number of actions (role access, permissions, available content, asset type, etc) use an overflow menu instead of changing action button icons per row.
...
- When a table has rows that include 1 or 2 action buttons, use individual actions buttons on hover.
...
- When a table has rows that include more than 2 action buttons, use an overflow menu instead.
When icons aren't able to fully represent the action behind the icon, use text buttons or an overflow menu instead.
- When a table has rows that include actions that are dependent on other properties (role access. permissions, available content, asset type, etc) use an overflow menu instead of changing action button icons per row.
...
- Actions will be disabled if a data table has batch edit activated.
- Users can only interact with table row actions if the batch edit is disabled/in normal viewing mode.
...
- Ensure that even if an action button is only available on hover, that we include the ability to tab to it for users with screen readers and those actively using keyboards. As soon as a user tabs into a row, display all available actions as if the user is hovering over the row.
