...
Allowing the user to manage the table view, helps to enhance users’ workflow and increase efficiency. Unnecessary columns can be hidden and users can put focus on columns that provide the most value to them. The user should have the flexibility of selecting one or multiple columns to show, hide or rearrange the column order. 
...
1. Header: Includes a title, description and optional label. The title should be brief and clearly describe the user’s task or purpose. Use a number count in brackets to indicate how many columns are currently shown in the table display.
2. Search: To find columns in the list.
3. Title: The user can select all columns at once by selecting the checkbox.
...
If no changes were made, the primary action button is disabled until the user has modified something in the tearsheet.  After the user makes a change, the “Save”-button turns from disabled to active:
...
If a user makes changes, columns should stay in place while the user edits. After the user closes the tearsheet, the dialogue updates so that the selected columns are at the top. 
...
For large amount of columns in data tables a search bar is required, so that the user can easily find specific columns and the user can to enter a search term. In general, it is recommended to add a search bar when a table has more than 12 columns, or items are not visible in the viewport anymore.
...
The user gets a visual indication of the dragging starting point and the target.

![Reorder drag](../images/column-customization/image_Reorder-drag.png)


When the item is placed in the new desired state, the save button turns active because a change has been made. The user can save the reorder and also receive a confirmation message afterward.
...
In some use cases, we need to allow users to edit column values in the tearsheet while doing other modifications such as reorder. The user gets icons for editing indicated by hovering over the row. Follow the guidance for [https://pages.github.ibm.com/cdai-design/pal/components/data-table/inline-editing](url).

When the user needs the ability to view (or edit) further column info such as the data type, a second column can be introduced to list the info next to the column name. 
...
For many applications, the user needs a way to load more elements in the list as there can be thousands of items and loading them all initially and in some cases can slow down the loading of the tearsheet. Users should not need to select a 'load more' button to view more items while in a selection mode. Therefore, their intent to look for more items in the list (scrolling) is used to trigger an automatic loading of more items when they pass 2/3 of the available scroll area. The header and search bar are fixed on scroll and the list content scrolls behind. Loading is indicated by a skeleton representing three list entries. Usually, column loading shouldn’t take long – only rare cases need skeletons here.
...
Follow the guidance described for tearsheets. The styling and interactions of the list follow the Carbon data table rows.
