...
The progressive loading component is _not_ to be confused with the [Progress indicator](https://www.carbondesignsystem.com/components/progress-indicator/usage) or any other step wizard pattern, but should be considered alongside the [Carbon loading pattern](https://www.carbondesignsystem.com/patterns/loading-pattern/).

Once triggered, the loading bar will begin by showing an indeterminate _back and forth_ motion while data is being gathered during the first stages of the load. This bar then changes to a determinate indicator (finishing from left to right) when enough information is gathered to complete the load animation. The final determinate stage of the load is used to visually indicate the completion of the task to the user.
