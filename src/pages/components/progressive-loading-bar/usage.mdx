...
There are two types of loading bars: indeterminate and determinate.
...
- A loading spinner or skeleton load isn’t suitable
...
2. **The track** The static part of the component. This is the ‘runway’ for the bar.
   Color: `$ui-03`
3. **Percentage indicator (optional)** If you are can provide this additional information and it would be beneficial for the user, then show the real-time percentage of the load as it completes.
   Text style: `$body-short-01`, `$text-02` (right aligned to the edge of the track, 16px below).

### Placement and width

The loading bar would appear at the top of a container — or at least above the information that is being impacted — spanning the full width. The loading bars ideally won’t span the entire width of a page, unless the entire page is being updated and a spinner and/or skeleton loads aren’t suitable. The bar should span the width of a side panel or card, or the editable zone of a tearsheet, with no margins around it.

In this example, the loading bar appears when the user has chosen a different currency, meaning that all of the pricing information displayed needs to be converted to accurately reflect the new selection. The loading bar is placed above the information affected by the load and will disappear when the load has been completed.
...
- Use progressive loading for every single step in a flow. This could make the experience feel longer than it is.
...
<Row>
  ...
  <DoDontExample type="do" caption="Make sure the track spans the entire width of where it is placed (for example, the top of a card).">
