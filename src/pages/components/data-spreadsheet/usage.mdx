...
<ul>
    ...
    <li>Use to perform complex mathematical operations or analysis with content.</li>
    <li>Use to perform actions (manipulations) on the content displayed such as creating a new column, reorganizing columns, renaming columns, etc.</li>
    ...
    <li>Use to customize the presentation of content displayed such as cells' color, size, etc.</li>
    <li>Use to display content that can have an absence of value—`empty`, beyond `null`, `NA`, `––`. etc.</li>
</ul>
...
<ul>
    ...
    <li>Do not use to optionally replace data tables without consideration of the behaviors or functionality such as the ability to reorder or modify content.</li>
    <li>Do not use if row actions are required (i.e. overflow menu to delete or pause items).</li>
</ul>
...
1. **Cells:** A cell holds data and is formed by the intersection of a column and row, forming a rectangle along a grid of other cells.

2. **Headers:** A special type of cell. They are usually the top or leftmost cell in a column or row that acts as an identifier of that column or row. These typically act as `sticky` in that they stay stuck to the top of the component when scrolling.
...
Data spreadsheets typically use the same column and row headers as the height of the cells. However, the headers' heights may also act independently of the rows' heights and cells' heights. 
...
Attempt to keep the same alignment throughout the entire column. This makes it easier to scan vertically. Content is by default top aligned, but can be altered for specific cases.
...
Do not attempt to include too much in a cell. Too much variety of content (i.e. string + icon + symbol + button + accordion + emoji + divider) will cause discomfort to viewers when scanning down columns.
...
Headers main content include text, but may also includes buttons and icons.
...
Users can select single or multiple cells, single or multiple columns, single or multiple rows, or a combination of any of the three. Some tools may dictate how selection behaves. For example, tools that cannot currently edit individual cells restrict selection to whole columns only. For read-only use cases, selection can be used to copy content or reveal insights to selections through context panels.

Selection uses 2 types of focus border thickness: 1px and 2px.
...
When selecting a column or row, the first cell in the sequence becomes the active cell while the remaining are only selected. The header changes to an active state.
...
When selecting multiple columns or rows, the last column or row's first cell in the sequence becomes the active cell. The headers change to an active state.
...
Rows being deleted would shift the remaining rows up. No need to display a warning, but giving users the ability to undo or reset values to their original status is then required. 

Columns being deleted would shift remaining columns to the left. No need to display a warning, but giving users the ability to undo or reset values to their original status is then required.

Alternately, an optional warning modal may be presented if the action of deleting is non-reversible.
...
To move a single column or row, select the column and drag it to the desired location. A blue line will appear in the location where the column will be placed.
...
Vertical and horizontal scrolls are always available in the spreadsheet unless otherwise specified. The content the dictates the length of scrollbar and the scroll-area. When there is no data, limit the number of columns and rows that users can see by scrolling.
...
This acts as what is expected, an image or call-to-action in the cell's content area. Scenarios that include this traditional empty state are:
...
For the color, we use the `Blue hue` in Carbon for the Light theme and the `Cyan hue` for the Dark theme. You can see more of this in the styles section.
...
A column overflow menu usually affects the contents in that column. Occasionally when a user selects something such as "sorting" options from a column, it may rearrange the contents of the spreadsheet according to that column. Otherwise, all actions should affect the selected column only.
...
- Use top-left as the default alignment placement.
- Context menus and overflow menus should use a different color layer than the cells per theme. For example, If cells are using a gray10, the overflow menu should be colored white.
- Do not use buttons inside cells. Use the entire cell as the clickable area for an action.
- If a type of action (button, dropdown, etc.) exists in the header, the minimum height of the header should be 32px.
