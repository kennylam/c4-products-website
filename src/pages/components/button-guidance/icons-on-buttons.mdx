...
An exception to this rule are universal actions using either XL (64px) or 2XL (80px) buttons (for example, in tearsheets and modals), as buttons of this size do not support the use of icons.

#### Don't use an icon in the universal list for another action
Using icons in the above list for other actions can cause confusion around the expected outcome/experience.
...
#### If one or more buttons in a group has an icon, it doesn't mean they all need to have an icon
...
_The don't example uses the icon 'document export' for the publish action, which isn't suitable. As publish doesn't have its own well-defined icon, it's most appropriate to use a text-only button._
...
Some icons in the Carbon library have different variations, such as alt, filled and outlined. As not all icons have variations, we should be using the "default" option for all icons (the only exception are status icons, which have their own defined icon to use). Default icons are simply named after their action.
...
1. The icon must be standardized and recognizable without text, or represents an action with a strong visual attribute – such as a printer icon to represent a print action.
