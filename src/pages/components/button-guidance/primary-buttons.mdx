...
## Overview 
Primary buttons should only appear once per screen (the only exceptions are temporary primary flows). This helps give the user focus and gives them context behind the expected next step.
...
To identify the primary action on a page with multiple actions, ask yourself: "Which action would a user most likely need to do on this page?" The primary action often aligns with a common action, such as Create or Add. 

Sometimes, when answering this question, it feels like there is still more than the primary button needed. This might indicate you need to use a temporary primary flow in your design. 

## Temporary primary flows
There may be times when there is a primary button on a page, but through interaction with the page content, another flow may be launched that has its primary button. Temporarily, there may be two primary buttons on a page. 
